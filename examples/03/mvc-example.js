...
var selectionModel = {
    ...
    //アイスクリーム個数を変更する
    changeIcecreamNumber: function(num) { 
        if (num > 0 && num < 4) {
            this.icecreamNumber = num;
            this.clear();
        }
    },
    
    getIcecreamNumber: function() {
        return this.icecreamNumber;
    },
...
//VIEW: 個数ラジオボタン更新するView
function updateIcecreamNumber() {
    $("form input[name='icecream-number']").each(
        function(i,elm) {
            elm.checked = (elm.value == selectionModel.getIcecreamNumber());
        });
}
...
$(function () {
    var els = $('#icecreams');
      ...
      $("input[name='icecream-number']").click(function() { selectionModel.changeIcecreamNumber(parseInt(this.value,10));});
  });
...
function testModels() {
	...
	selectionModel.changeIcecreamNumber(3);
    ...
    ok("selectionModel:3つ目を追加した時の個数",selectionModel.getIcecreams().length,3);
    ok("selectionModel.contain:3つ目を追加した時のチェック",true,selectionModel.contain(all[2]));

    selectionModel.add(all[3]);
    ok("selectionModel:4つ目を追加した場合の",selectionModel.getIcecreams().length,3);
    ok("selectionModel.contain:3つ目を追加した時のチェック",true,selectionModel.contain(all[3]));
